...
/* Maple Mini configuration: PB1 LED (onboard), PB8 config button, PB9 USB disconnect */
static const device_config_t default_device_config = {
    ...
    .usb_disconnect_pin = { .port = GPIOB, .pin = 9, .dir = gpio_dir_output, .speed = gpio_speed_low, .func = gpio_func_general, .output = gpio_output_od, .polarity = gpio_polarity_high },
    .cdc_config = {
        .port_config = {
            /*  Port 0 */
            {
            ...
            /*  Port 1 */
            {
                .pins = 
                {
                    ...
                    /* dcd */ { .port = 0 }, /* Removed - conflicts with PB8 config button */
                    /*  ri */ { .port = GPIOB, .pin = 12, .dir = gpio_dir_input,  .pull = gpio_pull_up, .polarity = gpio_polarity_low },
                    /* txa */ { .port = 0 }, /* Removed - conflicts with PB1 status LED */
                }
            },
            /*  Port 2 */
            {
                .pins = 
                {
                    ...
                    /* dcd */ { .port = 0 }, /* Removed - conflicts with PB9 USB disconnect */
